...
(function ($) {
  'use strict'
    ...
    $(".delete-btn").click(async function(e){
        ...
        const post_id = $(this).attr("href");

        await Swal.fire({
            title: 'هشدار!',
            text: "آیا می خواهید پست مورد نظر را حذف کنید ؟",
            icon: 'warning',
  ...
  if (d){
    ...
    if (query.date){
      $('.normal-example').val(query.date.replaceAll('%2F','/'));
    }
  }

  $(".show-user-info").click(async function () {
    ...
    $.ajax({
        ...
        success:async function (response) {
            if (response.statusCode == 200){
              await Swal.fire({
                title: '<p style="direction: rtl;">'+response.user.name+'</p>',
                html: '<p style="direction: rtl;">ایمیل کاربر : '+response.user.email+'</p><p style="direction: rtl;">دستگاه کاربر : '+response.user.device+'</p><p style="direction: rtl;">تاریخ عضویت : '+response.user.joint_date+'</p><p style="direction: rtl;">تعداد نظرات : '+response.commentCount+'</p><p style="direction: rtl;">تعداد پسند ها : '+response.likeCount+'</p>',
        ...
        error:async function (error) {
          await Swal.fire({
            title: 'هشدار',
            text: 'خطایی در دریافت داده ها رخ داده است'
          });
        }
    });
  });
